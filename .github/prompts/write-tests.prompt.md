---
description: Write tests for a feature or change. Single-shot.
---
...
**Single-shot:** Infer `{name}`, change summary, and runner from git diff + `docs/ai/` + Active Stack Profile. **At most one** question only if there is no diff and no brief match.

1. **Gather Context** — Derive scope from `git diff` / status and feature docs; note **Assumed:** for flaky-test warnings if none documented.

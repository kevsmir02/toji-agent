---
name: ux-design
description: UX behaviour rules — block-based information architecture, progressive disclosure, navigation structure, form flow, empty/loading/error state behaviour, and aria-live accessibility patterns. Use when designing or reviewing user flows, multi-step forms, navigation hierarchies, command menus, or interaction states. Do NOT use for visual token decisions like colors, spacing values, or typography (use ui-reasoning-engine for those).
globs: ["**/*.tsx","**/*.jsx","**/*.html","**/*.vue","**/*.blade.php"]
---

This skill applies a functional minimalist UX framework focusing on structured architecture, progressive disclosure, and clarity. It complements `ui-reasoning-engine` by governing information flow, interaction density, and behavioral clarity.
...
- Treat meaningful UI elements as self-contained blocks or modules.
- When using Archetype A (from blueprints.md), elements should behave as drag-and-drop functional blocks. For other archetypes (B and C), focus on logical grouping instead of draggable blocks.
- Design information and actions as composable modules with clear boundaries.
...
- Every list or table feature must include a dedicated Empty State structure with purpose text and a primary next action.
- Every async action must use a Skeleton Loader that mirrors the final structure and rhythm.
- Every hidden menu action must have a corresponding documented keyboard shortcut.

*(Note: Visual token decisions like spacing grids, color, and font sizes are governed exclusively by `ui-reasoning-engine`.)*
